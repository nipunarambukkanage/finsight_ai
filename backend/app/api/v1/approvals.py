...
async def list_pending_approvals(user: TokenPayload = Depends(get_current_user)):
...
async def get_approval_details(approval_id: str, user: TokenPayload = Depends(get_current_user)):
...
@router.post("/{approval_id}/request-changes", response_model=ApprovalRecord)
async def request_changes(
    approval_id: str,
    req: DecisionRequest,
    user: TokenPayload = Depends(get_current_user),
):
    """Record a distinct atomic request-changes decision."""
    try:
        return approval_service.process_decision(
            approval_id=approval_id,
            reviewed_by=user.sub or "demo_analyst",
