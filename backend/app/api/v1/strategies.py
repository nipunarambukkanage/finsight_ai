...
async def execute_strategy_backtest(strategy_id: str, req: RunBacktestRequest, user: TokenPayload = Depends(get_current_user)):
...
async def start_shadow_trading(
    ...
    approver = user.sub or "demo_analyst"
    try:
...
async def pause_shadow_trading(strategy_id: str, simulation_id: str, user: TokenPayload = Depends(get_current_user)):
...
async def get_strategy_performance(strategy_id: str, ticker: str = "AAPL", user: TokenPayload = Depends(get_current_user)) -> Dict[str, Any]:
